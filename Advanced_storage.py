...
# Get a list of column names and types# Get a 
...
engineengine..executeexecute('SELECT * FROM measurement LIMIT 5').fetchall()
...
from sqlalchemy.ext.automapsqlalch  import automap_base
...
df_last12months_precipitation.head(35)
...
# Query to calculate sum of all 'prcp' for each weather station for previous year dates# Query 
...
query_to_runquery_t  = f"SELECT station, sum(prcp) FROM measurement WHERE date >= '{lastyear_start_date}' AND date <= '{lastyear_end_date}' "            "GROUP BY station "            "ORDER BY sum(prcp) DESC"
print(query_to_run)
